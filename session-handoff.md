...
- Processed 18 DOCX web links into ignored local PDFs, now organized under the categorized RAG raw corpus layout in `rag/data/raw/news/`, using 4 direct report PDFs and 14 cleaned browser-printed fallbacks
- Imported 19 provided local PDFs plus 18 rendered web PDFs into ignored `rag/data/processed/normalized_documents.jsonl`
- Rebuilt normalized corpus after seven problematic CoinShares PDFs were manually replaced; those report PDFs are now organized under the categorized RAG raw corpus layout in `rag/data/raw/news/`
...
- RAG web PDF download: `cd rag && python -m src.ingestion.download_web_pdfs --links-docx /path/to/web-links.docx --output-dir data/raw/news --manifest data/raw/web_pdf_manifest.jsonl`
...
- RAG real BGE sample retrieval: `cd rag && HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 MILVUS_COLLECTION=cryptopulse_rag_chunks_bge_m3_sample EMBEDDING_MODEL_NAME=/path/to/bge-m3-snapshot .venv/bin/python -c "from src.retrieval import retrieve; print(retrieve('What does Aave V3 improve?', top_k=3))"`
- RAG full BGE hybrid retrieval: `cd rag && HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 MILVUS_COLLECTION=cryptopulse_rag_chunks_bge_m3_full EMBEDDING_MODEL_NAME=/path/to/bge-m3-snapshot BM25_INDEX_PATH=data/processed/bm25_index.json .venv/bin/python -c "from src.retrieval import retrieve; print(retrieve('Aave V3 capital efficiency', top_k=3))"`
...
- Chatbot full no-mock check: `cd chatbot && export RAG_SITE_PACKAGES=/path/to/CryptoPulse/rag/.venv/lib/python3.11/site-packages && export PYTHONPATH="$RAG_SITE_PACKAGES:$PYTHONPATH" && USE_MOCK=false RAG_USE_MOCK=false LLM_BACKEND=lora NER_BACKEND=lora LORA_USE_MOCK=false LORA_REMOTE_BASE_URL=http://127.0.0.1:6006/v1 LORA_REMOTE_API_KEY=$LORA_REMOTE_API_KEY USE_MILVUS_NATIVE_HYBRID=true USE_CROSS_ENCODER_RERANKER=false MILVUS_COLLECTION=cryptopulse_rag_hybrid_bge_m3_bm25 EMBEDDING_MODEL_NAME=/path/to/bge-m3-snapshot RERANK_MODEL_NAME=/path/to/bge-reranker-base BM25_INDEX_PATH=../rag/data/processed/bm25_index.json HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 RAG_SYSTEM_SITE_PACKAGES=/path/to/python3.11/site-packages .venv/bin/uvicorn src.app:app --host 127.0.0.1 --port 8000`
